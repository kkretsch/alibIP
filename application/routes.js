...
module.exports = function(app, passport, myConnectionPool) {
	...
	app.get('/ext/lookup_ical', function(req, res) {
		...
		async.series({
			...
			getter: function(callback) {
				// Which sort of filter?
				var sQuery='SELECT *,DATE_FORMAT(ts, \'%Y%m%dT%H%i%s\') AS icalts, DATE_FORMAT(tsrefresh, \'%Y%m%dT%H%i%s\') AS icaltsrefresh, DATE_FORMAT(CONVERT_TZ(ts, \'+01:00\', \'+00:00\'), \'%Y%m%dT%H%i%sZ\') AS icalstamp FROM entries WHERE fkuser=?';
				var aQuery=[aGrant.fkuser];
				if(aGrant.fkentry) {
					sQuery+=' AND id=?';
					aQuery.push(aGrant.fkentry);
