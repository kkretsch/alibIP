...
module.exports = function(app, passport, myConnectionPool) {
	...
	app.get('/my/entries/:pagelen/:pagenum', function(req, res, next) {
		...
		myConnectionPool.query('SELECT id,ts,ipv4,ipv6,tsrefresh,countrefresh,DATE_FORMAT(ts, \'%Y-%m-%d %H:%i:%s\') AS uts,DATE_FORMAT(tsrefresh, \'%Y-%m-%d %H:%i:%s\') AS utsrefresh FROM entries WHERE fkuser=? ORDER BY ts DESC LIMIT ?,?', [req.user.id,iFrom,req.pagelen], function(err, rows) {
			if (err) {
				console.log(err);
